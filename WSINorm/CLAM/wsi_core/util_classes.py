...
class isInContourV4_Easy(Contour_Checking_fn):
    def __init__(self, contour, patch_size):
        self.cont = contour
        self.patch_size = patch_size
        self.shift = (patch_size // 2 * np.arange(0, 1.01, 0.25)).astype(int)

    def __call__(self, pt):
        center = (pt[0] + self.patch_size // 2, pt[1] + self.patch_size // 2)
        all_points = []
        for sh in self.shift:
            all_points += [(center[0] - sh, center[1] - sh),
                           (center[0] + sh, center[1] + sh),
                           (center[0] + sh, center[1] - sh),
                           (center[0] - sh, center[1] + sh)
                           ]
