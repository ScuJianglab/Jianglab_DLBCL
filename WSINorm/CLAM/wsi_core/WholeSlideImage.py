...
import itertools
from .util_classes import isInContourV1, isInContourV2, isInContourV3_Easy, isInContourV3_Hard, Contour_Checking_fn, isInContourV4_Easy
...
#         self.name = ".".join(path.split("/")[-1].split('.')[:-1])
    ...
    def _getPatchGenerator(self, cont, cont_idx, patch_level, save_path, patch_size=256, step_size=256, custom_downsample=1,
        ...
        if isinstance(contour_fn, str):
            if contour_fn == 'multi_pt':
                cont_check_fn = isInContourV4_Easy(contour=cont, patch_size=ref_patch_size[0])
