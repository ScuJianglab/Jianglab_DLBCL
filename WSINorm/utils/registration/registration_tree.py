# from https://github.com/ChristianMarzahl/WsiRegistration/blob/main/qt_wsi_reg/registration_tree.py
import itertools
import pickle
import math
import random
...
class TriangleRegistrationPair:
    ...
    def __init__(self, source: Triangle, target: Triangle, homography: bool = True) -> None:
        ...
        self.tf_param_inv = tf.AffineTransformation(self.get_homography_inv[:2, :2],
                                                    self.get_homography_inv[:2, 2:].reshape(-1))
    ...
    def __setstate__(self, state):
        ...
        self.tf_param = tf.AffineTransformation(self.__dict__["homography"][:2, :2],
                                                self.__dict__["homography"][:2, 2:].reshape(-1))
        self.tf_param_inv = tf.AffineTransformation(self.__dict__["homography_inv"][:2, :2],
                                                    self.__dict__["homography_inv"][:2, 2:].reshape(-1))
        ...
        for rp1, rp2 in [wn_ws, es_ws, en_wn, en_ws]:
            ...
            if z != 0:  # lines are not parallel
                ...
                if self.contains(p):  # check that the point is a the border of the rectangle
                    ...
                    if Rect.create(Rect, x=min(p1.x, p1.x), y=min(p1.y, p1.y), w=abs(p1.x - p2.x),
                                   h=abs(p1.y - p2.y)).contains(p):
...
class RegistrationQuadTree:
    ...
    def __init__(self, source_slide_path: Path, target_slide_path: Path, source_boundary: Rect = None,
                 target_boundary: Rect = None,
                 depth=0, target_depth=1, thumbnail_size=(2048, 2048),
                 run_async=False, node_orientation: NodeOrientation = NodeOrientation.TOP,
                 parent=None, homography: bool = True, filter_outliner: bool = False, num_workers: int = 2,
                 initial_rotation=None, **kwargs):
        """[summary]
        ...
        if self.run_async:  # self.run_async
            with concurrent.futures.ThreadPoolExecutor() as executor:  # ProcessPoolExecutor  ThreadPoolExecutor
                source_func = functools.partial(RegistrationQuadTree.get_region_thumbnail,
                                                slide_path=self.source_slide_path, boundary=self.source_boundary,
                                                depth=self.depth + 1, size=self.thumbnail_size)
                target_func = functools.partial(RegistrationQuadTree.get_region_thumbnail,
                                                slide_path=self.target_slide_path, boundary=self.target_boundary,
                                                depth=self.depth + 1, size=self.thumbnail_size)

                submitted = {executor.submit(func): area for func, area in
                             zip([source_func, target_func], ["source", "target"])}
            ...
            self.source_thumbnail, self.source_scale = RegistrationQuadTree.get_region_thumbnail(
                slide_path=self.source_slide_path, boundary=self.source_boundary, depth=self.depth + 1,
                size=self.thumbnail_size)
            self.target_thumbnail, self.target_scale = RegistrationQuadTree.get_region_thumbnail(
                slide_path=self.target_slide_path, boundary=self.target_boundary, depth=self.depth + 1,
                size=self.thumbnail_size)

        # if the initial rotation angle is to big rotate the image to make the matching easier 
        inv_rotation_matrix = None
        if self.initial_rotation is not None and int(self.initial_rotation) not in range(-12,
                                                                                         12):  # tolerance of 5°
            inv_rotation_matrix = self.get_inv_rotation_matrix_from_angle(-self.initial_rotation,
                                                                          width=self.target_thumbnail.width,
                                                                          height=self.target_thumbnail.height)
            # rotate image 
            self.target_thumbnail = self.target_thumbnail.rotate(self.initial_rotation, fillcolor=(255, 0, 0),
                                                                 expand=True)

        self.ptsA, self.ptsB, self.matchedVis, self.mean_reg_error, self.sigma2, self.q, self.tf_param, self.mean_reg_error = self.perform_registration(
            self.source_thumbnail, self.target_thumbnail, source_scale=self.source_scale,
            target_scale=self.target_scale, inv_rotation_matrix=inv_rotation_matrix, **kwargs)
    ...
    def perform_registration(self, source_thumbnail, target_thumbnail, source_scale, target_scale, filter_outliner,
                             homography, inv_rotation_matrix=None, **kwargs):

        ptsA, ptsB, matchedVis = self.extract_matching_points(source_thumbnail, target_thumbnail,
                                                              source_scale=source_scale, target_scale=target_scale,
                                                              inv_rotation_matrix=inv_rotation_matrix, **kwargs)

        if filter_outliner:
            ptsA, ptsB, scale_factors = self.filter_outliner(ptsA, ptsB)
    ...
    def source_thumbnail(self):
        ...
        if self._source_thumbnail is None:
            self.source_thumbnail, self.source_scale = self.get_region_thumbnail(self.source_slide,
    ...
    def target_thumbnail(self):
        ...
        if self._target_thumbnail is None:
            self._target_thumbnail, self.target_scale = self.get_region_thumbnail(self.target_slide,
    ...
    def source_slide_dimensions(self, dimensions):
        ...
        if self.nw is not None:
            self.nw.source_slide_dimensions = dimensions
        if self.ne is not None:
            self.ne.source_slide_dimensions = dimensions
        if self.se is not None:
            self.se.source_slide_dimensions = dimensions
        if self.sw is not None:
            self.sw.source_slide_dimensions = dimensions
    ...
    def target_slide_dimensions(self, dimensions):
        ...
        if self.nw is not None:
            self.nw.target_slide_dimensions = dimensions
        if self.ne is not None:
            self.ne.target_slide_dimensions = dimensions
        if self.se is not None:
            self.se.target_slide_dimensions = dimensions
        if self.sw is not None:
            self.sw.target_slide_dimensions = dimensions
    ...
    def divide(self):
        ...
        p2h = self.transform_boxes([(east_edge, north_edge + (south_edge - north_edge) / 2, 50, 50)])[0][
              :2]  # center right

        p3 = self.transform_boxes([(east_edge, south_edge, 50, 50)])[0][:2]  # bottom right
        p3h = self.transform_boxes([(west_edge + (east_edge - west_edge) / 2, south_edge, 50, 50)])[0][
              :2]  # center bottom

        p4 = self.transform_boxes([(west_edge, south_edge, 50, 50)])[0][:2]  # bottom left
        p4h = self.transform_boxes([(west_edge, north_edge + (south_edge - north_edge) / 2, 50, 50)])[0][
              :2]  # center left

        pc = self.transform_boxes(
            [(west_edge + (east_edge - west_edge) / 2, north_edge + (south_edge - north_edge) / 2, 50, 50)])[0][
             :2]  # center

        # set new box coordinates withhin old limits
        p1, p2, p3, p4 = [(max(p[0], 0), max(p[1], 0)) for p in [p1, p2, p3, p4]]
        p1, p2, p3, p4 = [(min(p[0], self.target_slide_dimensions[0]), min(p[1], self.target_slide_dimensions[1])) for p
                          in [p1, p2, p3, p4]]
        ...
        if source_nw.is_valid() and target_nw.is_valid():
            qt_functions[NodeOrientation.NORTH_WEST] = functools.partial(self.__class__,
                                                                         source_slide_path=self.source_slide_path,
                                                                         target_slide_path=self.target_slide_path,
                                                                         source_boundary=source_nw,
                                                                         target_boundary=target_nw,
                                                                         depth=self.depth + 1,
                                                                         node_orientation=NodeOrientation.NORTH_WEST,
                                                                         parent=self, **self.kwargs)

        if source_ne.is_valid() and target_ne.is_valid():
            qt_functions[NodeOrientation.NORTH_EAST] = functools.partial(self.__class__,
                                                                         source_slide_path=self.source_slide_path,
                                                                         target_slide_path=self.target_slide_path,
                                                                         source_boundary=source_ne,
                                                                         target_boundary=target_ne,
                                                                         depth=self.depth + 1,
                                                                         node_orientation=NodeOrientation.NORTH_EAST,
                                                                         parent=self, **self.kwargs)

        if source_se.is_valid() and target_se.is_valid():
            qt_functions[NodeOrientation.SOUTH_EAST] = functools.partial(self.__class__,
                                                                         source_slide_path=self.source_slide_path,
                                                                         target_slide_path=self.target_slide_path,
                                                                         source_boundary=source_se,
                                                                         target_boundary=target_se,
                                                                         depth=self.depth + 1,
                                                                         node_orientation=NodeOrientation.SOUTH_EAST,
                                                                         parent=self, **self.kwargs)

        if source_sw.is_valid() and target_sw.is_valid():
            qt_functions[NodeOrientation.SOUTH_WEST] = functools.partial(self.__class__,
                                                                         source_slide_path=self.source_slide_path,
                                                                         target_slide_path=self.target_slide_path,
                                                                         source_boundary=source_sw,
                                                                         target_boundary=target_sw,
                                                                         depth=self.depth + 1,
                                                                         node_orientation=NodeOrientation.SOUTH_WEST,
                                                                         parent=self, **self.kwargs)

        if self.run_async == True:
            with concurrent.futures.ThreadPoolExecutor(
                    max_workers=self.num_workers) as executor:  # ProcessPoolExecutor  ThreadPoolExecutor
    ...
    def draw_feature_points(self, num_sub_pic: int = 5, figsize=(16, 16), patch_size: int = 512):
        ...
        # quiver plot
        try:
            ptA_quiver = self.scale_points(
                [(pt + (self.source_boundary.west_edge, self.source_boundary.north_edge)).astype(int) for pt in
                 np.copy(self.ptsA)], self.source_scale, operator.truediv)
            ptB_quiver = self.scale_points(
                [(pt + (self.target_boundary.west_edge, self.target_boundary.north_edge)).astype(int) for pt in
                 np.copy(self.ptsB)], self.target_scale, operator.truediv)
            ...
            u, v = (ptA_quiver[:, 0] - ptB_quiver[:, 0] + 0.0001) / u_max, (
                    ptA_quiver[:, 1] - ptB_quiver[:, 1] + 0.0001) / v_max
        ...
        for idx, (pA, pB) in enumerate(zip(self.ptsA[:num_sub_pic].copy(), self.ptsB[:num_sub_pic].copy())):
            ...
            transformed = (tf_temp.transform(transformed) + (
                self.target_boundary.west_edge, self.target_boundary.north_edge)).astype(int)
            pA = pA.astype(int)

            size_target_x, size_target_y = int(size * self.mpp_x_scale), int(size * self.mpp_y_scale)

            image_source, image_target, image_target_trans = np.zeros(shape=(1, 1)), np.zeros(shape=(1, 1)), np.zeros(
                shape=(1, 1))
            ...
            if size_target_x > 0 and size_target_y > 0:
                image_target_trans = target_slide.read_region(location=trans_location, level=0,
                                                              size=(size_target_x, size_target_y))
                image_target_trans = image_target_trans.rotate(self.get_rotation_angle, fillcolor=(255, 255, 255),
                                                               expand=True)
        ...
        if self.divided:
            figures = [fig]
            if self.nw is not None:
                figures.append(self.nw.draw_feature_points(num_sub_pic, figsize)[0])
            if self.ne is not None:
                figures.append(self.ne.draw_feature_points(num_sub_pic, figsize)[0])
            if self.se is not None:
                figures.append(self.se.draw_feature_points(num_sub_pic, figsize)[0])
            if self.sw is not None:
                figures.append(self.sw.draw_feature_points(num_sub_pic, figsize)[0])
    ...
    def filter_boxes(self, boxes):
        ...
        boxes = boxes[
            ((boxes[:, 1] > self.source_boundary.north_edge) & (boxes[:, 1] < self.source_boundary.south_edge))]
    ...
    def transform_boxes(self, boxes, max_depth=100):
        ...
        for box in boxes:
            ...
            if self.nw is not None and self.nw.source_boundary.contains(point) and self.nw.depth <= max_depth:
                ...
                transformed_xy = tf_temp.transform(source_boxes) + (self.target_boundary.west_edge,
                                                                    self.target_boundary.north_edge)
    ...
    def draw_annotations(self, boxes, figsize=(16, 16), num_sub_pic: int = 5):
        ...
        for idx, (source_box, target_box) in enumerate(zip(source_boxes[:num_sub_pic], target_boxes[:num_sub_pic])):
            ...
            target_anno_width, target_anno_height = int(source_anno_width * self.mpp_x_scale), int(
                source_anno_height * self.mpp_y_scale)
            target_x1, target_y1 = (size_target_x / 2) - target_anno_width / 2, (
                    size_target_y / 2) - target_anno_height / 2
            ...
            trans_location = transformed - (size_target_x // 2, size_target_y // 2)
            if size_target_x > 0 and size_target_y > 0:
                image_target_trans = target_slide.read_region(trans_location, level=0,
                                                              size=(size_target_x, size_target_y))
        ...
        if self.divided:
            sub_draws = []
            if self.nw is not None:
                sub_draws.append(self.nw.draw_annotations(boxes, figsize, num_sub_pic))
            if self.ne is not None:
                sub_draws.append(self.ne.draw_annotations(boxes, figsize, num_sub_pic))
            if self.se is not None:
                sub_draws.append(self.se.draw_annotations(boxes, figsize, num_sub_pic))
            if self.sw is not None:
                sub_draws.append(self.sw.draw_annotations(boxes, figsize, num_sub_pic))
    ...
    def _get_detector_matcher(self, point_extractor="orb", maxFeatures: int = 500, crossCheck: bool = False,
                              flann: bool = False, **kwargs):

        kwargs.update(
            {"point_extractor": point_extractor, "maxFeatures": maxFeatures, "crossCheck": crossCheck, "flann": flann})
    ...
    @staticmethod
    def _filter_matches(kp1, kp2, matches, ratio=0.75, **kwargs):
    ...
    def extract_matching_points(self, source_image, target_image,
        ...
        if callable(point_extractor):
            ...
            kpsA_ori, descsA = detector.detectAndCompute(source_image,
                                                         None) if use_gray == False else detector.detectAndCompute(
                cv2.cvtColor(source_image, cv2.COLOR_BGR2GRAY), None)
            kpsB_ori, descsB = detector.detectAndCompute(target_image,
                                                         None) if use_gray == False else detector.detectAndCompute(
        ...
        if inv_rotation_matrix is not None:
            tf_angle_target_inv = tf.AffineTransformation(inv_rotation_matrix[:2, :2],
                                                          inv_rotation_matrix[:2, 2:].reshape(-1))
            kpsB = tf_angle_target_inv.transform(kpsB)

        ptsA, ptsB = self.scale_points(kpsA, source_scale, operator.mul), self.scale_points(kpsB, target_scale,
                                                                                            operator.mul)
        return ptsA, ptsB, matchedVis

    @staticmethod
    def scale_points(kps, scale, op=operator.mul):
    ...
    def get_region_thumbnail(slide_path: Path, boundary: Rect, depth: int = 0, size=(2048, 2048)):
        ...
        downsample = max(
            *[dim / thumb for dim, thumb in zip((boundary.w, boundary.h), (size[0] * depth, size[1] * depth))])
        level = slide.get_best_level_for_downsample(downsample)

        downsample = slide.level_downsamples[level]

        x, y, w, h = int(boundary.west_edge), int(boundary.north_edge), int(boundary.w / downsample), int(
            boundary.h / downsample)
